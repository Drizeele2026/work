...
test("只修改发布当天值班人的 OpenID 时，当天值班人不变且结果带新 OpenID", () => {
  const remote = {
    version: 2,
    current: {
      teams: [
        {
          ...
          color: "violet",
          members: [
            { name: "许绵绵", feishuOpenId: "ou_xu_old" },
            { name: "郑成清", feishuOpenId: "ou_zheng_old" },
            { name: "谭贤", feishuOpenId: "ou_tan_old" }
          ]
        }
      ]
    },
  ...
  const nextTeams = [
    {
      ...
      members: [
        { name: "许绵绵", feishuOpenId: "ou_xu_old" },
        { name: "郑成清", feishuOpenId: "ou_zheng_new" },
        { name: "谭贤", feishuOpenId: "ou_tan_old" }
      ]
    }
  ];
  ...
  const document = utils.buildPublishedDocument(remote, nextTeams, {
    publishDateKey: "2026-07-02",
    updatedAt: "2026-07-02T00:00:00.000Z"
  });
  const todayDuty = utils.findAssignmentForDateWithFallback(document, "2026-07-02").teams[0];
