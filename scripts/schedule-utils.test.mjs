...
test("发布当月只改名单时沿用已有计划，未变团队整月不重排", () => {
  const oldRemote = {
    config: {
      teams: [
  ...
  assert.deepEqual(
    ...
    [
      ...
      "2026/07/07 前端:林胜聪 | 后端:杨朋举 | 测试:许绵绵"
    ]
  );
});

test("发布当月名单变化时可自动加当天节点，让变更团队从当天按新名单顺排", () => {
  const oldRemote = {
    config: {
      teams: [
        { name: "前端", members: ["郑刘利", "林颖", "林胜聪", "刘红辉", "王朋伟"], last: "郑刘利", color: "blue" },
        ...
        dailyAssignments: [
          {
            dateStr: "2026/07/03",
            teams: [
              { name: "前端", person: "刘红辉", color: "blue" },
              { name: "后端", person: "张凯", color: "green" },
              { name: "测试", person: "许绵绵", color: "violet" }
            ]
          },
          {
            dateStr: "2026/07/04",
            teams: [
              { name: "前端", person: "王朋伟", color: "blue" },
              { name: "后端", person: "张家南", color: "green" },
              { name: "测试", person: "郑成清", color: "violet" }
            ]
          }
        ]
  ...
  const anchoredTeams = utils.applyRosterChangeAnchors(nextTeams, oldRemote, { publishDateKey: "2026-07-04" });
  const testerAnchors = JSON.parse(JSON.stringify(anchoredTeams.find((team) => team.name === "测试").anchors));
  const frontendAnchors = JSON.parse(JSON.stringify(anchoredTeams.find((team) => team.name === "前端").anchors));

  assert.deepEqual(testerAnchors, [
    { date: "2026-07-01", mode: "previousDay", person: "钟右梅" },
    { date: "2026-07-04", mode: "currentDay", person: "郑成清" }
  ]);
  assert.deepEqual(frontendAnchors, [{ date: "2026-07-01", mode: "previousDay", person: "郑刘利" }]);

  const generated = utils.generateAssignmentsForMonth({ config: { teams: anchoredTeams }, months: {} }, 2026, 7);
  const monthEntry = {
    year: 2026,
    month: 7,
    teams: anchoredTeams,
  ...
  assert.deepEqual(
    Array.from(merged.dailyAssignments.slice(2, 7), (day) =>
      `${day.dateStr} ${day.teams.map((team) => `${team.name}:${team.person}`).join(" | ")}`
    ),
    [
      "2026/07/03 前端:刘红辉 | 后端:张凯 | 测试:许绵绵",
